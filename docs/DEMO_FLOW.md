...
pnpm balance       # agent wallet has ETH (gas) + USDC (for the real x402 step)
```
- [ ] `.env.local` has a real `NEXT_PUBLIC_PRIVY_APP_ID`
- [ ] AI key set (`OPENAI_API_KEY` or `ANTHROPIC_API_KEY`)
- [ ] Agent wallet funded on Base Sepolia (≥ 0.05 ETH, ≥ 1 USDC) + `X402_PAY_TO_ADDRESS` set
...
> work with no wallet and no testnet.** Only the single real x402 settlement needs funds; if it's
...
**Resume** → **Run purchase** → a **real x402 USDC payment settles on Base** (click the settlement
link), the trust ring pops up, toast shows **Trust +N**. "Real money, on-chain, gated by the
worker's own authorization — and you can see exactly why the score moved."
...
Sentinel is the operating system for the AI-native company."
...
- **Wallet won't connect** → check Privy **Allowed origins** includes your URL. (Auth isn't needed
  for the graph/scoring/A2A demo.)
